...
describe('state and deterministic routing', () => {
  ...
  it('fails a stalled run after inactivity and frees its slot', async () => { const prev = process.env.PATTY_RUN_INACTIVITY_MS; process.env.PATTY_RUN_INACTIVITY_MS = '30'; try { const store = new Store(); const a = account('a'); store.addAccount(a); const adapter = new ControlledAdapter(); const c = new Coordinator(store, new Router(store), new Map([[a.id, adapter]])); const runId = await c.start({ model: 'gpt-5-codex', input: 'x' }); await wait(); expect(store.account(a.id)?.activeRuns).toBe(1); await new Promise(resolve => setTimeout(resolve, 80)); expect(store.run(runId)?.status).toBe('failed'); expect(store.account(a.id)?.activeRuns).toBe(0); } finally { process.env.PATTY_RUN_INACTIVITY_MS = prev; } });
});
